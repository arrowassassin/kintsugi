# Kintsugi — Design Doc (solo edition, v1.1 — decisions locked)

A local-first safety layer for AI coding agents. Kintsugi intercepts the commands an agent
...
- Coverage uses the cheapest available interception point per agent: native hooks where they
  exist (e.g. Claude Code), the MCP `kintsugi-exec` tool for agents that call tools, and a
...
- Honestly scoped guarantee: Kintsugi cannot promise *nothing runs un-warned* (an agent can decline
...
```
        ...
        │  • MCP server `kintsugi-exec` (tool-calling agents)   │
        │  • $PATH shim (agents that shell out raw)          │
        └───────────────────┬───────────────────────────────┘
                            │ local socket (IPC)
            ┌───────────────▼──────────────────────────────┐
            │           Kintsugi resident daemon               │
...
1. Interception (adapter layer)
   ...
   - MCP server (`kintsugi-exec` tool agents call instead of raw bash).
...
4. Local model (Tier 2, explain + score the ambiguous band) — LOCKED
   ...
   - Fetched + checksummed on first run (not embedded in the binary). If a local Ollama exists,
     may use it in local/free mode — but Kintsugi never *requires* Ollama.
...
5. Snapshot + undo
   ...
   - `kintsugi undo` (last action), `kintsugi undo --session` (whole agent session).
6. Flight recorder
   - Append-only, hash-chained event log in SQLite (command, decision, tier, summary, snapshot ref).
   - `kintsugi log` query + signed export for audit/compliance.
7. Policy
   - Per-project `.kintsugi.toml` committed to the repo: allow/deny rules, risk threshold, mode.
   - Global user defaults in `~/.config/kintsugi/`.
...
9. Controls
   ...
   - Tamper-evidence: detect/record attempts to disable Kintsugi or rewrite the log.
...
- Phase 3 — Undo (2–3 wks): predicted-path snapshots + `kintsugi undo` / `--session`.
- Phase 4 — Recorder + backstop (2–3 wks): hash-chained log, FS-watcher backstop, `ratatui` timeline.
- Phase 5 — Polish & launch (1–2 wks): `.kintsugi.toml`, unattended mode, kill-switch, signed
...
- Beyond v1, the team/enterprise direction (central policy, off-box tamper-evident audit, SIEM
  ...
  [`kintsugi-enterprise-roadmap.md`](kintsugi-enterprise-roadmap.md) — additive and opt-in, with the
...
These govern an agent building Kintsugi unattended. They are intentionally strict — a tool whose
...
- Once Phase 1's gate works, run the autonomous build agent itself THROUGH Kintsugi, so any
