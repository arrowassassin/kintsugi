# Aegis — Phases 2–5 design & build plan

Continuation of `aegis-design-doc.md` and `aegis-phase0-1-tasklist.md`. Phases 0
...
## Phase 2 — `aegis-model` (explain + score the ambiguous band)
...
- Daemon: when a destructive command is **allowed** (rule allow, memory/policy
  ...
  on the event.
- CLI: `aegis undo` (last reversible action) and `aegis undo --session` (whole
...
Stated plainly in `aegis undo` output and docs.
...
- Off by default; `aegis watch <path>` or `.aegis.toml` opt-in (watching is
  resource-sensitive and must be a deliberate choice).

### TUI (`aegis-tui`, launched by `aegis tui`)
...
- **Panic kill-switch:** `aegis panic` writes a kill-state flag the daemon checks
  *first*, instantly forcing every decision to Deny/Hold (halts current + queued
  agent actions); `aegis resume` clears it. Recorded in the log.
- **`aegis init` polish:** `AEGIS_DATA_DIR`, clearer agent detection and output,
