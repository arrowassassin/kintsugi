...
use aegis_daemon::ipc::{Observation, Request, Resolution, Response};
...
fn accessors_report_defaults() {
    ...
    assert_eq!(d.mode(), aegis_core::Mode::Attended);
...
fn kill_switch_path_is_beside_the_db() {
    ...
    let p = aegis_daemon::kill_switch_path();
    assert_eq!(p, tmp.path().join(aegis_daemon::KILL_SWITCH_FILE));
    std::env::remove_var("AEGIS_DB");
}
...
fn ipc_messages_roundtrip_through_json() {
    ...
    let res = Request::Resolve(Resolution {
        command: aegis_core::ProposedCommand::new("t", "/tmp", vec!["ls".into()], "ls"),
        decision: aegis_core::Decision::Allow,
        remember: true,
    });
...
fn load_policy_defaults_when_absent() {
    ...
    let tmp = tempfile::tempdir().unwrap();
    std::env::set_var("AEGIS_CONFIG", tmp.path().join("none.toml"));
    let p = aegis_daemon::load_policy(tmp.path());
    assert_eq!(p.risk_threshold(), aegis_core::policy::DEFAULT_THRESHOLD);
