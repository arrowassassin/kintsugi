...
//! timeline and `aegis undo` stay complete — the honest guarantee is "nothing is
...
//!
//! Off by default; opt in with `aegis watch <path>`.
...
pub fn run(roots: &[PathBuf]) -> Result<()> {
    ...
    for root in roots {
        ...
        eprintln!("aegis-watch: watching {}", root.display());
    }

    for res in rx {
        match res {
            Ok(event) => forward(&event),
            Err(e) => eprintln!("aegis-watch: watch error: {e}"),
        }
    }
    Ok(())
}
...
fn forward(event: &notify::Event) {
    ...
    for path in &event.paths {
        ...
        if let Err(e) = Client::observe(&obs) {
            eprintln!("aegis-watch: could not record {}: {e}", path.display());
        }
    }
}
