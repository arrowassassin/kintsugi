...
use aegis_core::{shell, Decision, ProposedCommand};
...
pub const TOOL_NAME: &str = "aegis-exec";
...
fn initialize_response(id: Value, req: &Value) -> String {
    ...
    result_response(
        ...
        json!({
            ...
            "capabilities": { "tools": {} },
            "serverInfo": { "name": "aegis-exec", "version": crate::VERSION },
            "instructions": "Run shell commands via the aegis-exec tool so Aegis can guard and record them."
        }),
    )
}
...
fn tools_list_response(id: Value) -> String {
    result_response(
        ...
        json!({
            "tools": [{
                "name": TOOL_NAME,
                "description": "Run a shell command guarded and recorded by Aegis. \
...
fn exec_through_aegis(agent: &str, cwd: PathBuf, command: &str) -> ToolOutcome {
    ...
    let decision = match Client::send(&proposed) {
        ...
        Err(e) => {
            ...
            eprintln!("aegis-mcp: warning: daemon unreachable; running unguarded: {e}");
            Decision::Allow
        }
    };
...
mod tests {
    ...
    fn initialize_reports_server_info() {
        ...
        assert_eq!(resp["id"], 1);
        assert_eq!(resp["result"]["serverInfo"]["name"], "aegis-exec");
        // Echoes the client's requested protocol version.
        assert_eq!(resp["result"]["protocolVersion"], "2025-06-18");
    }

    #[test]
    fn tools_list_includes_aegis_exec() {
