...
use aegis_core::{Class, Decision, Verdict};
...
impl HookOutcome {
    /// Emit nothing, exit 0 — "Aegis has no opinion, use your default flow."
    pub fn silent() -> Self {
        Self {
            stdout: None,
            exit_code: 0,
...
impl Dialect {
    ...
    /// The `agent` tag stamped onto the [`aegis_core::ProposedCommand`] so the
...
/// one-click "allow" it would run it with no Aegis snapshot, voiding the
...
fn format_cursor(resolved: &Resolved) -> HookOutcome {
    ...
    if let Some(r) = reason {
        ...
        map.insert(
            "userMessage".into(),
            serde_json::json!(format!("Aegis: {r}")),
        );
