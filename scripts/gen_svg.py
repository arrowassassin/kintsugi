...
# CHARW is the monospace grid pitch in px at FS=15.
# DejaVu Sans Mono advance ≈9.0px; add a hair so borders flush cleanly.
...
def main():
    ...
    y = TOP + 4
    for line in lines:
        # One <text> per color segment, each anchored at its exact grid x and
        # forced to its exact grid width via textLength+spacingAndGlyphs.
        # This prevents box-drawing characters (─ │ ╭ ╮) from drifting when the
        # browser's font advance differs from CHARW — no gaps, no overlap.
        segs = colorize(line, rules)
        col = 0
        for txt, color in segs:
            ...
            x = PADX + col * CHARW
            w = len(txt) * CHARW
            # Spaces → non-breaking so SVG doesn't collapse runs of whitespace.
            escaped = html.escape(txt).replace(" ", "&#160;")
            p.append(
                f'<text x="{x:.1f}" y="{y}" font-size="{FS}" fill="{color}"'
                f' textLength="{w:.1f}" lengthAdjust="spacingAndGlyphs">{escaped}</text>'
            )
            col += len(txt)
        y += LINEH
    p.append("</svg>")
