# Aegis — Phase 0 & 1 build task list (hand this to Claude Code)
...
- Rust workspace, edition 2021+. Single shipped binary `aegis` plus a resident daemon.
...
aegis/
  ...
  CLAUDE.md
  crates/
    aegis-core/              # types, rule engine, policy, event log (no I/O side effects)
    aegis-daemon/            # resident process, IPC server, decision loop
    aegis-intercept/         # hook adapter, MCP server, $PATH shim
    aegis-cli/               # `aegis` binary: init, status, log, approve, undo (stub)
    aegis-model/             # Tier-2 model wrapper (stub in P0/P1; real in P2)
    aegis-tui/               # ratatui UI (P4; create empty now)
```

## Shared types (aegis-core) — define first
...
- Transport: Unix domain socket (`$XDG_RUNTIME_DIR/aegis.sock`) / Windows named pipe.
...
## Interception adapter contract (aegis-intercept)
...
P0.1  Scaffold the workspace and the six crates; `aegis --version` runs.
  - Accept: `cargo build` succeeds; `aegis` prints version.

P0.2  `aegis-core`: define the shared types above + an append-only SQLite event log
...
P0.3  `aegis-daemon`: start a socket server that accepts `ProposedCommand`, logs it, and
...
P0.4  `aegis-intercept` shim: a binary that, when symlinked as `rm`/`git`/etc. on a temp
...
P0.5  `aegis-intercept` Claude Code hook adapter: register an Aegis hook so Claude Code sends
...
P0.6  `aegis-intercept` MCP server `aegis-exec`: expose a tool agents (Qwen/Codex/custom) can
      call to run a command; it bridges to the daemon. Document how to point an agent at it.
  - Accept: an MCP call to `aegis-exec` runs the command and logs it tagged with the agent.

P0.7  `aegis-cli`: `aegis init` (auto-detect installed agents, wire hook/MCP/shim, start daemon),
      `aegis status`, `aegis log` (pretty timeline of recent events).
  - Accept: fresh machine → `aegis init` → run a command in any wired agent → `aegis log` shows it.
...
P1.1  `aegis-core` rule engine: classify a `ProposedCommand` into Safe / Catastrophic / Ambiguous.
...
P1.3  `aegis-cli` approval prompt: on Hold, print the "hold card" — risk class, the raw command
...
P1.4  Decision memory (`aegis-core`): per-repo always-allow / always-deny by command hash;
      consulted before rules.
  - Accept: an `[r]`'d command auto-allows on next run; recorded in the log as memory-allow.

P1.5  Policy file: read `.aegis.toml` from the repo root (allow/deny additions, mode) and
      `~/.config/aegis/` defaults; repo overrides global.
  - Accept: adding a deny rule in `.aegis.toml` causes that command to Hold/Deny.
...
P1.7  Demo: script the 30-second flow — agent proposes `rm -rf` → Aegis holds it → you press
...
- Phase 2: `aegis-model` real impl — bundle/download 3B Q4_K_M GGUF (1.5B low-RAM fallback,
  auto-selected), keep warm in the daemon, fill `summary`+`risk`, wire graduated unattended mode.
- Phase 3: snapshots + `aegis undo` (predict touched paths; reflink CoW + copy fallback).
- Phase 4: FS-watcher backstop + `ratatui` timeline reading the event log.
- Phase 5: kill-switch, signed binaries, `aegis init` polish, open-source launch.
