# Aegis — Admin-Locked Settings + Passive Session Recorder (design + market analysis)
...
1. **Password-locked, encrypted, admin-provisioned settings.** A sysadmin installs
   Aegis, sets an admin password, and locks protection so a regular user — or an AI
   agent running as that user — can't trivially weaken it. **Stopping/unhooking/
   disabling Aegis itself requires the password** when locked. Secret stored at the
...
- **Claude Code `/rewind` checkpointing** — auto-snapshots files *Claude edits*, restores
  ...
  shell case Aegis targets is the documented blind spot of the agent's own undo.* (code.claude.com/docs/en/checkpointing)
...
Aegis = *"Undo for Operators," generalized to the filesystem, made **per-dangerous-command**,
...
was taken first.** Aegis must never claim more:
...
The one mechanism available on **all three OSes is file-level reflink/clone**, which Aegis
...
The **daemon is the sole authority and sole log-writer.** Today `aegis stop` just reads a PID
...
- **Daemon runs as a dedicated `aegis` system user** (locked posture) so the audited user can't
...
> One honest sentence: *"Locked settings stop an AI agent or a normal user from weakening Aegis,
> and turn any forced shutdown into a logged, alerting, recoverable event. They do not stop root —
> root can disable any same-machine tool, and Aegis is designed to make that visible, not impossible."*
...
- **Recovery:** one-time recovery key at provision (FileVault/BitLocker pattern); lost pw + lost
  key → privileged local teardown only (no Aegis-held escrow — spine #5). Secret store unavailable
...
- **Primary:** an rc-file **preexec hook** — zsh native `add-zsh-hook preexec`, bash via a
  *vendored, checksum-pinned* `bash-preexec` (spine #5 forbids `curl|source`); `precmd`/`$?` for
  exit codes. **Fire-and-forget**: backgrounds a one-shot `aegis ingest` → daemon. Captures
...
- **Higher-assurance:** **`aegis record`** — a PTY-wrapping logged shell (like `script`/tlog) for
...
*gaps* detectable; auditd `-e 2`/eBPF is the root-backed floor Aegis **integrates with, not
reimplements** (no kernel code — CLAUDE.md).

### 5c. Secret redaction = LAUNCH BLOCKER (both DBA + infosec said so)
auditd writes DB passwords to the audit log; tlog disables input logging *because of it*. Aegis
...
- `aegis report --since <range> --destructive --by-user --format json|csv`
- `aegis log --danger --since 24h` · `aegis whoran <pattern>` · `aegis verify` (chain
  intact/broken/gapped) · `aegis undo coverage`. **All offline/local-first** (DB hosts are segmented).
...
- **Non-blocking, lossy-but-chained:** daemon-down → spool to `~/.aegis/spool/`, daemon folds it
...
- **P-A0 (this PR): foundation + redaction.** Command-line **secret redaction** module in
  `aegis-core` (launch-blocker, self-contained, reused by recorder + improves today's log) +
  this design doc. Fully unit-tested.
- **P-A1: locked-settings core.** `aegis-core::admin` — argon2id verifier + AEAD-sealed settings,
...
- **P-A2: daemon auth + password-to-stop.** `AuthBegin/Prove`, session token, rate-limit/lockout,
  privileged ops behind IPC, authenticated `aegis stop`, watchdog + `daemon-killed` event.
- **P-A3: recorder.** `Request::Record` + additive versioned columns; `aegis ingest`; rc preexec
