# Kintsugi — Enterprise / Team Roadmap (companion to the design doc)

> This is a **forward-looking** track, separate from the locked solo design doc
> (`kintsugi-design-doc.md`). Nothing here ships in v1. It exists so that, when team
> and enterprise demand arrives, we extend Kintsugi **without betraying the local-first
> tool solo users love, and without weakening the security spine.**

## The thesis

Today Kintsugi is a dev-first, local-first, single-machine tool. Its adoption engine
...
We do **not** chase "unbypassable prevention." Kintsugi stays a *detective + reversible*
...
- **Org-policy layer above local policy.** Precedence today is rules → policy →
  ...
  merge is monotonic). `kintsugi policy show --effective` prints rules vs local vs org
...
- **`kintsugi-relay` log shipper (opt-in).** Forwards append-only event-log rows to a
...
- *Deliverable:* `kintsugi-relay` + documented collector contract + cross-device chain
  verification.

### E2 — SIEM / SOC integration
- **Export formats:** OCSF (preferred) + CEF + JSON for Splunk, Elastic, Datadog,
  Sentinel. `kintsugi export --format ocsf`.
...
- **Enrollment:** `kintsugi enroll <token>` joins a machine to an org; devices pull the
...
- *Deliverable:* self-hostable control plane + `kintsugi enroll`.
...
- **Remote approval:** route a held command to a human approver via Slack / Teams /
  PagerDuty — a natural extension of the existing **approval queue** (`kintsugi queue` /
...
- **OSS core is free and local — permanently.** It is the credibility and adoption
  engine; nerfing it to upsell would kill the thing that makes Kintsugi spread.
...
E0 (monotonic org-policy layer + `policy show --effective`) **+** E1 (`kintsugi-relay`
